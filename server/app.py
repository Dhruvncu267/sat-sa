...
Endpoints:
  ...
  GET  /login.html                    -> the login screen (also has the
                                          public 'create an account' box)
  POST /api/signup                    -> open account creation, called from
                                          that box (public -- no token, no
                                          login required to create one)
...
_PUBLIC_PATHS = {"/", "/login.html", "/login.js", "/firebase-config.js", "/favicon.ico", "/api/signup"}
...
def api_signup():
    ...
    box right on login.html. Anyone who reaches this website can create
...
def api_add_report(entity_id):
    ...
    try:
        ...
        return jsonify({
            ...
            "risk_score": added["risk_score"], "totals": scoped["totals"],
        })
    except ingest.IngestError as e:
        return jsonify({"error": str(e)}), 400
