"""
Create a supervisor account for SOCAssure (Firebase version).
================================================================
This is an optional command-line alternative -- the app's login page has
an open "Create an account" box that does the same thing with no command
line at all (POST /api/signup in app.py), and there's also an older
token-gated /setup?token=... web page. This script needs your Firebase
credentials set up exactly like the running app does (see
...
def prompt_create():
    print("Create a new SOCAssure supervisor account.\n")
    ...
    try:
        ...
        sys.exit(1)
    print(f"\nDone. '{email}' can now log in to SOCAssure with the password you just set.")


def main():
    parser = argparse.ArgumentParser(description="Manage SOCAssure supervisor accounts.")
